...
{
    [[QuickbloxCumunicator sharedInstance] queryForNotifications:option onCompletion:^(NSArray *objects) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    QBCOCustomObject *notification = notificationArray[indexPath.row];
    if([[notification.fields objectForKey:@"type"] isEqualToString:@"contribute"]){
        nCell.notificationImageView.image = [UIImage imageNamed:@"contributer"];
        
        [QBRequest userWithID:[[notification.fields objectForKey:@"user_id"] integerValue] successBlock:^(QBResponse *response, QBUUser *user) {
            
            [QBRequest objectWithClassName:@"Story" ID:notification.parentID successBlock:^(QBResponse *response, QBCOCustomObject *object) {
                // do something with retrieved object
                QBCOCustomObject *story = object;
                
                nCell.notificationLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%@ contributet to tale %@",nil), user.login, [story.fields objectForKey:@"title"]];
                nCell.story = story;
    ...
    else if([[notification.fields objectForKey:@"type"] isEqualToString:@"like"]){
        ...
        [QBRequest userWithID:[[notification.fields objectForKey:@"user_id"] integerValue] successBlock:^(QBResponse *response, QBUUser *user) {
            ...
            [QBRequest objectWithClassName:@"Story" ID:notification.parentID successBlock:^(QBResponse *response, QBCOCustomObject *object) {
                ...
                nCell.notificationLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%@ liked tale %@",nil), user.login, [story.fields objectForKey:@"title"]];
                nCell.story = story;
            } errorBlock:^(QBResponse *response) {
                // error handling
                NSLog(@"Response error: %@", [response.error description]);
    ...
    else if([[notification.fields objectForKey:@"type"] isEqualToString:@"friend"]){
        nCell.notificationImageView.image = [UIImage imageNamed:@"contributer"];
        
        [QBRequest userWithID:[[notification.fields objectForKey:@"user_id"] integerValue] successBlock:^(QBResponse *response, QBUUser *user) {
            nCell.notificationLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%@ added you as a friend",nil), user.login];
            nCell.friendname = user.login;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([@"showStoryToNotification" compare: segue.identifier]==NSOrderedSame) {
        ...
        pageStoryViewController.story = cell.story;
    }
