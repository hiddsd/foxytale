...
-(UIImage*) drawText:(NSString*)text inImage:(UIImage*)image atPoint:(CGPoint)point numberOfLines:(NSUInteger)numberOfLines
{
    ...
    UIFont *font = [UIFont boldSystemFontOfSize:22]; //font size
    ...
    CGRect rect = CGRectMake(point.x + 5, coordinateY - 5, image.size.width - 5, image.size.height); //Text Zeichen in dieser Fläche
...
-(UIImage*) drawCoverText:(NSString*)text inImage:(UIImage*)image atPoint:(CGPoint)point numberOfLines:(NSUInteger)numberOfLines
{
    ...
    UIFont *font = [UIFont fontWithName:@"Verdana-Bold" size:40]; //font size
    [image drawInRect:CGRectMake(0,0,image.size.width,image.size.height)]; //Zeichenfläche
    
    NSDictionary *attributes = [[NSDictionary alloc] initWithObjectsAndKeys: font, NSFontAttributeName, nil];
    
    //Wieviel Zeilen belegt der Text?
    numberOfLines = [self sizeOfTextLine:text inRectWitdh:image.size.width-10];
    ...
    CGRect rect = CGRectMake(point.x + 5, coordinateY + 5, image.size.width - 10, image.size.height - 10); //Text Zeichen in dieser Fläche
...
-(int)sizeOfTextLine:(NSString*)text inRectWitdh:(float)with{
    
    CGRect r = [text boundingRectWithSize:CGSizeMake(2000, 0)
                                  options:NSStringDrawingUsesLineFragmentOrigin
                               attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Verdana-Bold" size:40]}
                                  context:nil];
    
    int numberOfLines = ceil(r.size.width / with);
